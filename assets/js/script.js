...
async function getAlbums() {
  let artistList = await fetch(`/assets/songs/`);
...
async function getSongs(folder) {
  ...
  let songList = await fetch(`/assets/songs/${currentArtist}/albums/${folder}/`);
...
async function displayAlbums(artist) {
  ...
  let as = await fetch(`/assets/songs/${artist}/albums/`);
  ...
  for (let index = 0; index < array.length; index++) {
    ...
    if (e.href.includes(`/albums/`)) {
      ...
      let a = await fetch(`/assets/songs/${artist}/albums/${albumfolder}/info.json`);
